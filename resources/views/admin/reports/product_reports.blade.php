    ...
    <div class="container-fluid p-4 p-lg-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">{{ __('Manage reported products') }}</p>
            </div>
        </div>
                                        ...
                                        <td class="text-end">
                                            <div class="btn-group" role="group">
                                                @if($report->status !== 'reviewed')
                                                    <form action="{{ route('admin.product-reports.update-status', [$report, 'reviewed']) }}"
                                                          method="POST"
                                                          class="d-inline product-report-status-form">
                                                        @csrf
                                                ...
                                                @if($report->status !== 'ignored')
                                                    <form action="{{ route('admin.product-reports.update-status', [$report, 'ignored']) }}"
                                                          method="POST"
                                                          class="d-inline product-report-status-form ms-1">
                                                        @csrf
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const forms = document.querySelectorAll('.product-report-status-form');

                forms.forEach(function (form) {
                    form.addEventListener('submit', function (e) {
                        e.preventDefault();

                        const toStatus = this.action.split('/').pop(); // last segment is status
