...
@section('content')
    ...
    <div class="container-fluid p-4 p-lg-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-0">{{ __('Edit Variant') }}</h1>
                <p class="text-muted mb-0">{{ __('Update variant information and options') }}</p>
            </div>
            <div>
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.variants.update', $variant) }}" method="POST" id="variantForm">
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input @error('is_active') is-invalid @enderror" 
                                           ...
                                           name="is_active" value="1" {{ old('is_active', $variant->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="mb-4">
                                <div class="form-check form-switch">
                                    <input class="form-check-input @error('is_required') is-invalid @enderror" 
                                           ...
                                           name="is_required" value="1" {{ old('is_required', $variant->is_required) ? 'checked' : '' }}>
                            ...
                            <div class="mb-4">
                                ...
                                <small class="text-muted d-block mb-3">{{ __('Add or update options for this variant') }}</small>

                                <div id="optionsContainer">
                                    @php
                                        $oldOptions = old('options', []);
                                        $existingOptions = $variant->options;
                                        
                                        // Merge old input with existing options
                                        if (empty($oldOptions) && $existingOptions->count() > 0) {
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-check-lg me-2"></i>{{ __('Update Variant') }}
                                </button>
...
@push('scripts')
<script>
let optionIndex = {{ old('options') ? count(old('options')) : ($variant->options->count() ?? 0) }};
