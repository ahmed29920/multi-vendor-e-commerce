...
@php
    $page = 'vendor_product_ratings';
@endphp

@section('title', __('Product Ratings'))
    ...
    <div class="container-fluid p-4 p-lg-4">
            ...
            <div>
                <nav aria-label="breadcrumb" class="mb-2">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ __('Product Ratings') }}</li>
                    </ol>
                </nav>
                <h1 class="h3 mb-0">{{ __('Product Ratings') }}</h1>
                <p class="text-muted mb-0">{{ __('Manage ratings on your products') }}</p>
            </div>
        </div>
                            ...
                            <tbody>
                                @foreach($ratings as $rating)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <form action="{{ route('vendor.product-ratings.toggle-visibility', $rating) }}"
                                                  method="POST"
                                                  class="d-inline vendor-product-toggle-visibility-form">
                                                @csrf
        ...
        @else
            <div class="text-center py-5">
                ...
                <p class="text-muted mt-3">{{ __('No product ratings found.') }}</p>
            </div>
        @endif
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const forms = document.querySelectorAll('.vendor-product-toggle-visibility-form');
                ...
                forms.forEach(function (form) {
                    form.addEventListener('submit', function (e) {
                        ...
                        const actionText = isVisible
                            ? '{{ __('This rating will be hidden from product details and averages.') }}'
                            : '{{ __('This rating will be visible in product details and averages.') }}';
