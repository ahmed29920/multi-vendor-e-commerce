                    ...
                    <div class="card-body">
                        @if(!$vendorUser)
                            <div class="alert alert-danger">
                                {{ __('Vendor user not found.') }}
                            </div>
                        @else
                        <form action="{{ route('vendor.vendor-users.update', $vendorUser->id) }}" method="POST" id="vendorUserForm">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-lg-8">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('user_type') is-invalid @enderror" id="user_type" name="user_type" required>
                                            <option value="owner" {{ old('user_type', $vendorUser->user_type ?? 'owner') === 'owner' ? 'selected' : '' }}>
                                                {{ __('Owner') }}
                                            </option>
                                            <option value="branch" {{ old('user_type', $vendorUser->user_type ?? 'owner') === 'branch' ? 'selected' : '' }}>
                                                {{ __('Branch') }}
                                            </option>
                                        </select>
                                        @error('user_type')
                                    ...
                                    <div class="mb-3" id="branch_selection" style="display: {{ old('user_type', $vendorUser->user_type ?? 'owner') === 'branch' ? 'block' : 'none' }};">
                                        <label for="branch_id" class="form-label">{{ __('Branch') }} <span id="branch_required" style="display: {{ old('user_type', $vendorUser->user_type ?? 'owner') === 'branch' ? 'inline' : 'none' }};">*</span></label>
                                        <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id">
                                            <option value="">{{ __('Select a branch') }}</option>
                                            @foreach($branches as $branch)
                                                <option value="{{ $branch->id }}"
                                                    {{ old('branch_id', $vendorUser->branch_id) == $branch->id ? 'selected' : '' }}>
                                                    {{ $branch->name }}
                                                </option>
                                            @endforeach
                                        </select>
